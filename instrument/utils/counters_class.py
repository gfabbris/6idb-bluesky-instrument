...
class CountersClass:
    ...
    def detectors(self, value):
        if value is not None:
            ...
            for item in value:
                if isinstance(item, str):
                    ...
                    if isinstance(item, int):
                        ch = getattr(
                            scaler1.channels, 'chan{:02d}'.format(item+1)
                        )
            ...
            scaler1.select_plot_channels(scalerd_list)
    ...
    def monitor(self, value):
        if value is not None:
            if isinstance(value, int):
                ch = getattr(scaler1.channels, 'chan{:02d}'.format(value+1))
                value = ch.s.name
            scaler1.monitor = value
            self._mon = scaler1.monitor
    ...
    def monitor_counts(self):
        return scaler1.preset_monitor.get()
